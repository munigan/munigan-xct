	...
	for framename, settings in pairs(x.db.profile.frames) do
		if specificFrame and specificFrame == framename or not specificFrame then
			...
			if x.frames[framename] then
				...
				f = CreateFrame("ScrollingMessageFrame", "MuniganXCT"..framename.."Frame", UIParent)
				f:SetSpacing(2)
				f:ClearAllPoints()
				f:SetMovable(true)
				f:SetResizable(true)
				...
				f.sizing = CreateFrame("Frame", "MuniganXCT"..framename.."SizingFrame", f)
				f.sizing.parent = f
				...
				f.moving = CreateFrame("Frame", "MuniganXCT"..framename.."MovingFrame", f)
				f.moving.parent = f
...
function x.StartConfigMode()
	...
	for framename, settings in pairs(x.db.profile.frames) do
		if settings.enabledFrame then
			...
			f.title:SetFont(LSM:Fetch("font", "Homespun (Munigan)"), 10, "MONOCHROMEOUTLINE")
			...
			f.width:SetFont(LSM:Fetch("font", "Homespun (Munigan)"), 20, "MONOCHROMEOUTLINE")
			...
			f.height:SetFont(LSM:Fetch("font", "Homespun (Munigan)"), 20, "MONOCHROMEOUTLINE")
			...
			f.position:SetFont(LSM:Fetch("font", "Homespun (Munigan)"), 20, "MONOCHROMEOUTLINE")
...
function x.ToggleConfigMode()
	if x.configuring then
		...
		StaticPopup_Show("MUNIGAN_XCT_CONFIGURING")
	...
	if x.configuring then
		...
		if x.testing then
			...
			if type(hidePopup) == "boolean" and hidePopup then
				...
				StaticPopup_Show("MUNIGAN_XCT_TESTMODE")
			end
		end
	end
end
...
function x.RestoreAllDefaults()
	...
	GameTooltip:Hide()
	StaticPopup_Show("MUNIGAN_XCT_RESET_SETTINGS")
end

-- Popups
StaticPopupDialogs["MUNIGAN_XCT_CONFIGURING"] = {
	text			= "Configuring MuniganXCT\nType: |cffFF0000/mct lock|r to save changes",
	timeout			= 0,
	whileDead		= 1,
...
StaticPopupDialogs["MUNIGAN_XCT_TESTMODE"] = {
	text			= "MuniganXCT Test Mode",
	timeout			= 0,
	whileDead		= 1,
...
StaticPopupDialogs["MUNIGAN_XCT_RESET_SETTINGS"] = {
	text			= "Are your certain you want to erase |cffFF0000ALL|r your MuniganXCT settings?",
	timeout			= 0,
	whileDead		= 1,

	button1			= "|cffFF0000ERASE ALL!!|r",
	button2			= CANCEL,
	OnAccept		= function() MuniganXCTDB = nil; ReloadUI() end,
...
StaticPopupDialogs["MUNIGAN_XCT_HIDE_IN_COMBAT"] = {
