...
function x:OnInitialize()
  if xCT or ct and ct.myname and ct.myclass then
    print("|cffFF0000WARNING:|r MuniganXCT cannot load. Please disable xCT in order to use MuniganXCT.")
    return
  end

  -- Load the Data Base
  self.db = LibStub('AceDB-3.0'):New('MuniganXCTDB', addon.defaults)
  ...
  LibDualSpec:EnhanceDatabase(self.db, "MuniganXCT")
  ...
  if self.db.profile.showStartupText then
    print("Loaded |cffFF8000Munigan|r - |cffFFFF00Combat Text|r. To configure, type: |cffFF0000/mct|r")
  end
...
local tips = {
  ...
  "The |cffFFFF00Hide Config in Combat|r option was added to prevent |cffFFFF00MuniganXCT|r from tainting your UI. It is highly recommended left enabled.",
  "|cffFFFF00MuniganXCT|r has several different ways it will merge critical hits. You can check them out in |cffFFFF00Spam Merer|r.",
...
ACD:AddToBlizOptions(AddonName.."Blizzard", "Munigan - Combat Text")

-- Register Slash Commands
x:RegisterChatCommand('mct', 'OpenMuniganCommand')
...
function x:OpenMuniganCommand(input)
  ...
  if lock or save then
    ...
    elseif x.configuring then
      x:SaveAllFrames()
      x.EndConfigMode()
      print("|cffFF8000MuniganXCT|r frames have been saved. Please fasten your seat belts.")
      StaticPopup_Hide("MUNIGAN_XCT_CONFIGURING")
    else
      x.ToggleConfigMode()

      print("|cffFF8000MuniganXCT|r You are now free to move about the cabin.")
      print("      |cffFF0000/mct lock|r      - Saves your frames.")
      print("      |cffFF0000/mct cancel|r  - Cancels all your recent frame movements.")
    end
  ...
  if string_match(string_lower(input),'cancel') then
    if x.configuring then
      x:UpdateFrames();
      x.EndConfigMode()
      print("|cffFF8000MuniganXCT|r canceled frame move.")
    else
      print("|cffFF8000MuniganXCT|r There is nothing to cancel.")
    end
    return
  end

  if string_lower(input) == 'help' then
    print("|cffFF8000MuniganXCT|r commands:")
    print("      |cffFF0000/mct lock|r - Locks and unlocks the frame movers.")
    print("      |cffFF0000/mct test|r - Attempts to emulate combat.")
    return
  end
  ...
  if string_match(string_lower(input), 'track %w+') then
    ...
    x:UpdatePlayer()
    print("|cffFF8000MuniganXCT|r tracking unit:", name or "default")
  ...
  if x.inCombat and x.db.profile.hideConfig then
    if not shownWarning then
      print("|cffFF8000MuniganXCT|r will open the |cff798BDDConfiguration Tool|r after combat.")
      shownWarning = true
      lastConfigState = true
    end
  else
...
-- Register Slash Commands
x:RegisterChatCommand('track', 'TrackMuniganCommand')
function x:TrackMuniganCommand(input)
